...
// ReSharper restore CheckNamespace
{
    ...
    /// </New>
    public class DsfDecisionMultipleCriteriaActivity : DsfActivityAbstract<string>, ICollectionActivity
    {
        #region Properties
        ...
        public DsfDecisionMultipleCriteriaActivity()
            : base("Switch Flow")
        {
            // Initialise all the properties here
            ResultsCollection = new List<DecisionTO>();
        ...
        protected override void OnExecute(NativeActivityContext context)
        {
            ...
            {
                Dev2Logger.Log.Error("DSFDecisionMultipleCriteria", exception);
            ...
            finally
            {
                ...
                if(hasErrors)
                {
                    DisplayAndWriteError("DsfDecisionMultipleCriteriaActivity", allErrors);
        ...
        void AddResultDebugInputs(IEnumerable<DecisionTO> resultsCollection, Guid executionId, IDataListCompiler compiler)
        {
            var indexCount = 1;
            foreach(var DecisionTO in resultsCollection)
            {
                DebugItem debugItem = new DebugItem();
                if(!String.IsNullOrEmpty(DecisionTO.SearchType))
                {
                    AddDebugItem(new DebugItemStaticDataParams("", indexCount.ToString(CultureInfo.InvariantCulture)), debugItem);
                    AddDebugItem(new DebugItemStaticDataParams(DecisionTO.SearchType, ""), debugItem);

                    if(!string.IsNullOrEmpty(DecisionTO.SearchCriteria))
                    {
                        var expressionsEntry = compiler.Evaluate(executionId, enActionType.User, DecisionTO.SearchCriteria, false, out errorsTo);
                        AddDebugItem(new DebugItemVariableParams(DecisionTO.SearchCriteria, "", expressionsEntry, executionId), debugItem);
                    }

                    if(DecisionTO.SearchType == "Is Between" || DecisionTO.SearchType == "Not Between")
                    {
                        var expressionsEntryFrom = compiler.Evaluate(executionId, enActionType.User, DecisionTO.From, false, out errorsTo);
                        AddDebugItem(new DebugItemVariableParams(DecisionTO.From, "", expressionsEntryFrom, executionId), debugItem);

                        var expressionsEntryTo = compiler.Evaluate(executionId, enActionType.User, DecisionTO.To, false, out errorsTo);
                        AddDebugItem(new DebugItemVariableParams(DecisionTO.To, " And", expressionsEntryTo, executionId), debugItem);
                    }
        ...
        void InsertToCollection(IEnumerable<string> listToAdd, ModelItem modelItem)
        {
            ...
            {
                DecisionTO DecisionTO = ResultsCollection.Last(c => !c.CanRemove());
                var startIndex = ResultsCollection.IndexOf(DecisionTO) + 1;
                foreach(var s in listToAdd)
                {
                    mic.Insert(startIndex, new DecisionTO(s, ResultsCollection[startIndex - 1].SearchType, startIndex + 1));
                    startIndex++;
                }
        ...
        void AddToCollection(IEnumerable<string> listToAdd, ModelItem modelItem)
        {
            ...
            foreach(var s in listToAdd)
            {
                mic.Add(new DecisionTO(s, searchType, startIndex + 1));
                startIndex++;
            }
            CleanUpCollection(mic, modelItem, startIndex);
        }
        ...
        {
            return ResultsCollection.Count(DecisionTO => !DecisionTO.CanRemove());
        }

        public IList<DecisionTO> ResultsCollection { get; set; }
